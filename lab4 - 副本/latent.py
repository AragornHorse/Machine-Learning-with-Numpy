...
encoder = mlp.MLP(28 ** 2, h_s, [256], 0., mlp.LeakyRelu, norm=False)
decoder = mlp.MLP(h_s, 28 ** 2, [256], 0., mlp.LeakyRelu, norm=False, tail_act=mlp.Sigmoid())

loader = data.DataLoader(data.train_x / 255, data.train_y, batch_size=256)

opt = mlp.Adam(encoder.layers + decoder.layers, lr=1e-2, l2=0.)
...
for i in range(600):
    ...
    dec = decoder.forward(enc)

    loss = loss_func.forward(dec, x) / 28 ** 2 * 255
    # print(np.max(dec), np.max(x))

    dy = loss_func.backward()

    dy = decoder.backward(dy)
    dx = encoder.backward(dy)
...
plt.show()
plt.imshow(dec[0].reshape([28, 28]))
plt.show()
...
betas = np.linspace(0.005, 0.02, T)
alphas = 1 - betas
...
model = mlp.MLP(h_s + d_model * 2, h_s, [2048], dropout=0., act=mlp.Relu, norm=False, end_norm=False)
# model = mlp.Sequence(
...
lrs = np.concatenate([np.full([100, 1], 1e-3), np.linspace(1e-3, 1e-4, 100).reshape([100, 1]), np.linspace(1e-4, 3e-5, 100).reshape([100, 1]), np.linspace(3e-5, 1e-6, 1000).reshape([-1, 1])], 0).reshape([-1])
...
for epoch in range(300):
    ...
    for i in range(10):
        ...
        print("epoch{}: {}".format(epoch, loss / h_s))
...
x = generate(x[:, :], model) * B

x = decoder.forward(x, False)

np.save(r"./dif.npy", x)

plt.imshow((x[0].reshape([28, 28]) + 0.5) * 255)
plt.show()
